/*
  Endpoints used:
    ...
    GET /api/clients            -> customer list for search dropdown
    GET /api/sales/client/{clientKey} -> orders for a specific customer
...
function Sales() {
  ...
  // Fetch sales, revenue and customers 
  useEffect(() => {
    if (!token) return;
    Promise.all([
  ...
  // Filter customers by search term for dropdown
  ...
  const handleSelectCustomer = (customer) => {
    ...
    fetchCustomerOrders(customer.clientKey);
  };
  ...
  useEffect(() => {
    ...
    const top10 = [...sales]
      ...
      .slice(0, 10)
      .map(s => ({ order: s.orderNumber, amount: s.salesAmount || 0 }));
    ...
    const plot = Plot.plot({
      ...
      marks: [
        ...
        Plot.text(top10, {
          x: "amount", y: "order",
          text: (d) => `€${d.amount.toLocaleString()}`, dx: 8,
          fill: isDark ? "#e2e8f0" : "#1a2a6c",
          fontSize: "11px", fontWeight: "600",
        }),
        Plot.ruleX([0]),
      ],
      x: { label: "Sales Amount (€)", grid: true, tickPadding: 6, tickFormat: d => `€${(d/1000).toFixed(0)}K` },
      y: { label: null },
      style: { fontSize: "11px", color: t.textSecondary, background: "transparent" },
    });
  ...
  const renderOrderRow = (s) => {
    ...
    return (
      <tr key={s.orderNumber} style={{ borderBottom: `1px solid ${t.borderLight}` }}>
        <td style={{ ...styles.td, color: t.textSecondary, fontFamily: "monospace" }}>{s.orderNumber}</td>
        <td style={{ ...styles.td, color: t.textSecondary }}>{s.orderDate}</td>
        <td style={{ ...styles.td, color: isLate ? t.danger : t.textSecondary }}>{s.shipDate}</td>
        <td style={{ ...styles.td, color: t.textSecondary }}>{s.dueDate}</td>
        <td style={{ ...styles.td, color: t.textSecondary }}>{shipDays != null ? `${shipDays}d` : "-"}</td>
        <td style={{ ...styles.td, color: t.textSecondary }}>{s.quantity}</td>
        <td style={{ ...styles.td, color: t.textSecondary }}>€{s.price?.toLocaleString()}</td>
        <td style={{ ...styles.td, color: t.textPrimary, fontWeight: "600" }}>€{s.salesAmount?.toLocaleString()}</td>
      </tr>
    );
  };
 
  if (!user) {
    return <div style={{ ...styles.stateBox, color: t.textSecondary }}>Please sign in to view sales data.</div>;
  }
  ...
  return (
    <div style={styles.wrapper}>
      ...
      <div style={styles.chartsGrid}>
        <div style={{ ...styles.chartCard, background: t.cardBg, border: `1px solid ${t.border}` }}>
          ...
          {sales.length > 0 ? (
            ...
            <div style={{ ...styles.stateBox, color: t.textSecondary }}>{loading ? "Loading..." : "No data"}</div>
          )}
        </div>
      ...
      <div style={{ ...styles.filterBar, background: t.cardBg, border: `1px solid ${t.border}` }}>
        ...
        <div style={styles.customerSearchWrapper}>
          <input
            type="text"
            ...
            onChange={(e) => {
              ...
              if (!e.target.value) clearCustomerFilter();
            }}
          ...
          {showDropdown && filteredCustomers.length > 0 && (
            <div style={{ ...styles.dropdown, background: t.cardBg, border: `1px solid ${t.border}` }}>
              {filteredCustomers.map((c) => (
                <div
                  key={c.clientKey}
                  style={{ ...styles.dropdownItem, color: t.textPrimary }}
                  onClick={() => handleSelectCustomer(c)}
                  onMouseEnter={e => e.currentTarget.style.background = isDark ? "#334155" : "#f0f2f7"}
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                >
                  <span style={{ fontWeight: "500" }}>{c.firstName} {c.lastName}</span>
                  <span style={{ ...styles.dropdownSub, color: t.textSecondary }}>{c.country}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={styles.filterGroup}>
          ...
          <input
            ...
            style={{ ...styles.dateInput, background: t.inputBg, border: `1px solid ${t.border}`, color: t.textPrimary }}
          />
        </div>
        <div style={styles.filterGroup}>
        ...
        <button
          onClick={() => { setSearchTerm(""); setDateFrom(""); setDateTo(""); setMinAmount(""); setMaxAmount(""); clearCustomerFilter(); }}
      ...
      {selectedCustomer && (
        <div style={{ ...styles.tableCard, background: t.cardBg, border: `1px solid ${t.border}` }}>
          ...
          {customerLoading ? (
            <div style={{ ...styles.stateBox, color: t.textSecondary }}>Loading customer orders...</div>
          ) : customerOrders.length === 0 ? (
            <div style={{ ...styles.stateBox, color: t.textSecondary }}>No orders found for this customer.</div>
          ) : (
            <div style={styles.tableWrapper}>
              <table style={styles.table}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${t.border}` }}>
                    {["Order No.", "Order Date", "Ship Date", "Due Date", "Ship Time", "Quantity", "Price", "Sales Amount"].map((h) => (
                      <th key={h} style={{ ...styles.th, color: t.textSecondary }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {customerOrders.map(renderOrderRow)}
                </tbody>
              </table>
            </div>
          )}
      ...
      {/* All Sales Table - hidden when customer is selected */}
      {!selectedCustomer && (
        <div style={{ ...styles.tableCard, background: t.cardBg, border: `1px solid ${t.border}` }}>
          <div style={styles.tableHeader}>
            <div>
          ...
          {loading && <div style={{ ...styles.stateBox, color: t.textSecondary }}>Loading sales data...</div>}
          {error && !loading && <div style={{ ...styles.stateBox, color: t.danger }}>Error: {error}</div>}
