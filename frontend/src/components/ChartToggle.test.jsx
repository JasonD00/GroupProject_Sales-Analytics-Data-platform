/*
  ...
  Note: Observable Plot renders SVG directly into the DOM via
  chartRef, so these tests focus on the toggle button behaviour
  and available options rather than pixel-level chart output.
*/
...
describe('ChartToggle', () => {
  ...
  // ChartToggle normalises tier casing internally, so the upgrade hint
  // works correctly whether "Growth"/"Pro" (capitalised) or "GROWTH"/"PRO"
  // (uppercase, the format AuthContext actually uses) is passed in.
  ...
  it('switches the active chart type when a toggle button is clicked', () => {
    ...
    // The clicked button should now be visually active — we check this
    // indirectly by confirming it no longer throws and remains in the document
